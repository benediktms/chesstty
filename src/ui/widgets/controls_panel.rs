use crate::app::{AppState, GameMode};
use ratatui::{
    buffer::Buffer,
    layout::Rect,
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let mut lines = vec![
            Line::from(vec![
                ...
                Span::raw("Type square (e.g., e2)"),
            ]),
            Line::from(vec![
                Span::styled("      ", Style::default()),
                Span::raw("then destination (e4)"),
            ]),
            Line::raw(""),
            Line::from(vec![
            ...
            Line::from(vec![
                ...
                Span::raw(format!("{}", format_game_mode(&self.app_state.mode))),
            ]),
        ];
        ...
        lines.push(Line::raw(""));
        lines.push(Line::from(vec![
            Span::styled("Turn: ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            Span::styled(
                turn_text,
                Style::default().fg(if matches!(turn_color, cozy_chess::Color::White) {
                    Color::White
                } else {
                    Color::Gray
        ...
        if !matches!(status, cozy_chess::GameStatus::Ongoing) {
            lines.push(Line::raw(""));
            lines.push(Line::from(vec![
                Span::styled("Game: ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
                Span::styled(
        ...
        if let Some(ref engine_info) = self.app_state.ui_state.engine_info {
            lines.push(Line::raw(""));
            lines.push(Line::from(vec![
                Span::styled("Engine: ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
            ]));

            if let Some(depth) = engine_info.depth {
                lines.push(Line::from(vec![
                    Span::raw("  Depth: "),
                    Span::styled(depth.to_string(), Style::default().fg(Color::White)),
                ]));
            }
            ...
            if let Some(ref score) = engine_info.score {
                lines.push(Line::from(vec![
                    Span::raw("  Eval: "),
                    Span::styled(format_score(score), Style::default().fg(Color::White)),
                ]));
            }
        }
