...
pub fn parse_uci_message(line: &str) -> Result<UciMessage, crate::engine::UciError> {
    ...
    match tokens.first() {
        ...
        _ => Err(crate::engine::UciError::UnknownMessage(line.to_string())),
    }
}

/// Parse an "info" line from the engine
fn parse_info_line(tokens: &[&str]) -> Result<EngineInfo, crate::engine::UciError> {
...
pub fn parse_uci_move(s: &str) -> Result<Move, crate::engine::UciError> {
    if s.len() < 4 {
        return Err(crate::engine::UciError::InvalidMove(s.to_string()));
    }
...
fn parse_square(s: &str) -> Result<Square, crate::engine::UciError> {
    if s.len() != 2 {
        return Err(crate::engine::UciError::InvalidSquare(s.to_string()));
    }
    ...
    let file = match s.chars().next().unwrap() {
        ...
        'g' => File::G,
        'h' => File::H,
        _ => return Err(crate::engine::UciError::InvalidSquare(s.to_string())),
    };
