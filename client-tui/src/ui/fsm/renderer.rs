    ...
    fn render_overlay(
        ...
        match overlay {
            ...
            Overlay::ExpandedPanel { component } => {
                ...
                // Render mini board in corner when panel is expanded
                let mini_width = 20u16;
                let mini_height = 11u16;
                if area.width >= mini_width + 2 && area.height >= mini_height + 2 {
                    let mini_area = Rect {
                        x: area.x + area.width - mini_width,
                        y: area.y + area.height - mini_height,
                        width: mini_width,
                        height: mini_height,
                    };
                    let is_flipped = matches!(
                    ...
                    frame.render_widget(mini_board, mini_area);
                }
            }
        }
    }
