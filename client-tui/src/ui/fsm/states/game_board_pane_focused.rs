...
#[derive(Clone, Debug)]
pub struct GameBoardPaneFocusedState {
    pub focused_component: Component,
    pub render_spec: RenderSpec,
    pub controls: Vec<Control>,
}

impl Default for GameBoardPaneFocusedState {
    fn default() -> Self {
        let mut state = Self {
            focused_component: Component::HistoryPanel,
...
impl GameBoardPaneFocusedState {
    pub fn new(component: Component) -> Self {
        let mut state = Self {
            focused_component: component,
            render_spec: RenderSpec::game_board_with_pane(component),
            controls: Vec::new(),
        };
        state.controls = state.derive_controls();
        state
