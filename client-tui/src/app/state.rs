...
impl AppState {
    ...
    pub fn is_engine_turn(&self) -> bool {
        ...
        tracing::trace!("is_engine_turn: {}, side_to_move: {:?}, mode: {:?}",
            result, self.game.side_to_move(), self.mode);
        result
    }

    /// Make a move with the engine
    #[tracing::instrument(level = "info", skip(self), fields(move_num = self.game.history().len() + 1))]
    pub async fn make_engine_move(&mut self) -> Result<(), String> {
        tracing::info!("make_engine_move called");

        if !self.is_engine_turn() {
            tracing::debug!("Not engine's turn, skipping");
            return Ok(());
        }

        tracing::info!("Engine's turn to move");

        // Check if game is over
        let status = self.game.status();
        if !matches!(status, cozy_chess::GameStatus::Ongoing) {
            tracing::info!("Game is not ongoing (status: {:?}), not making engine move", status);
            return Ok(());
        }

        tracing::debug!("Setting status message to 'Engine thinking...'");
        self.ui_state.status_message = Some("Engine thinking...".to_string());

        // Get current position as FEN
        let fen = self.game.to_fen();
        tracing::debug!("Current FEN: {}", fen);

        // Make sure we have an engine
        if self.engine.is_none() {
            tracing::error!("Engine not initialized but make_engine_move was called");
            return Err("Engine not initialized".to_string());
        }
        tracing::debug!("Engine is initialized");
        ...
        tracing::info!("Calculated movetime: {}ms (skill_level: {})", movetime, self.skill_level);
        ...
        tracing::debug!("Sending SetPosition command to engine");
        {
            ...
            engine
                ...
                .await
                .map_err(|e| {
                    tracing::error!("Failed to send SetPosition command: {}", e);
                    e
                })?;
        }
        tracing::debug!("SetPosition command sent successfully");
        ...
        // Start calculation
        tracing::debug!("Sending Go command to engine");
        {
            let engine = self.engine.as_ref().unwrap();
            engine
                ...
                .await
                .map_err(|e| {
                    tracing::error!("Failed to send Go command: {}", e);
                    e
                })?;
        }
        tracing::info!("Go command sent successfully, engine should now be calculating");
            ...
            while let Some(event) = engine.try_recv_event() {
                match event {
                    EngineEvent::BestMove(mv) => {
                        tracing::info!("Received bestmove from engine: {:?}", mv);
                        return Some(mv);
                    }
                    EngineEvent::Info(info) => {
                        tracing::trace!("Received engine info update: score={:?}, depth={:?}, nodes={:?}",
                            info.score, info.depth, info.nodes);
                        self.ui_state.engine_info = Some(info);
                    }
                    EngineEvent::Ready => {
                        tracing::debug!("Engine sent Ready event");
                    }
                    _ => {
                        tracing::trace!("Received other engine event: {:?}", event);
                    }
                }
            }
        }
